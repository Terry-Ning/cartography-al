...
def plot_statistics(args: argparse.Namespace):
    ...
    strategies = ["bald", "cartography", "discriminative", "entropy", "leastconfidence", "random", "iq-bi-cls", "iq-regression", "iq-ratio", "iq-epi-alea"]
    # strategies = ["iq-bi-cls", "iq-regression", "iq-ratio"]
    ...
    for measurement in measurement_list:
        ...
        markers = {"random"     : "P", "entropy": "s", "leastconfidence": "^", "bald": "d", "discriminative": "X",
                "cartography": "o", "iq-bi-cls": '*', "iq-regression": '>', "iq-ratio": 'v',"iq-epi-alea": '<'}
        ...
        ax = sns.lineplot(data=result_dict,
                        ...
                        hue="strategy",
                        style="strategy",
                        style_order=["random", "entropy", "leastconfidence", "bald", "discriminative", "cartography", "iq-bi-cls", "iq-regression", "iq-ratio", "iq-epi-alea"],
                        # style_order=[ "iq-bi-cls", "iq-regression", "iq-ratio"],
                        # hue_order=["iq-bi-cls", "iq-regression", "iq-ratio"],
                        hue_order=["random", "entropy", "leastconfidence", "bald", "discriminative", "cartography", "iq-bi-cls", "iq-regression", "iq-ratio", "iq-epi-alea"],
                        markers=markers,
                        palette=pal,
                        ci=None)
        ax.grid(True) 
        ...
        ax.legend(fancybox=True, shadow=True, title="Sampling strategy", loc="center left", bbox_to_anchor=(1.0, 0.0),
                ncol=1)
        plt.tight_layout()
        plt.savefig(f"{os.getenv('STATISPLOT_PATH')}{args.task}/{args.task}_results_{measurement}.pdf", dpi=300)
        plt.close()