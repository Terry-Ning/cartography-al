...
def get_distribution_weights_iq(train: np.ndarray) -> torch.FloatTensor:
    train = train.tolist()
    train_0 = [label for label in train if label == 0]
    train_1 = [label for label in train if label == 1]
    train_2 = [label for label in train if label == 2]
    logger.debug(f"Class distribution -- train: {len(train_0)} 0's, {len(train_1)} 1's, {len(train_2)} 2's")

    n_samples = [len(train_0), len(train_1), len(train_2)]
    normed_weights = [1 - (x / sum(n_samples)) for x in n_samples]
    normed_weights = torch.FloatTensor(normed_weights)
